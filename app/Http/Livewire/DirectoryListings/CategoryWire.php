...
class CategoryWire extends Component
{
    ...
    public function performAction()
    {
        if ($this->action === 'trash') {
            $directoryListing =  Category::whereIn('id', $this->selected)->get();
    ...
    public function render()
    {
        ...
        $directoryListingCategories = Category::where('menu_id',3)->latest('updated_at')->paginate(5);
        $directoryListingCategories = Category::query()->where('menu_id',3)->latest('updated_at');
        
               foreach ($this->searchColumns as $column => $value) {
                   if (!empty($value)) {
                   $directoryListingCategories->when($column == 'name', fn($directoryListingCategories) => $directoryListingCategories->where('categories.' . $column, 'LIKE', '%' . $value . '%'));
                   }
               } 
        
               return view('livewire.directory-listings.category-wire',  [
                   'directoryListingCategories' => $directoryListingCategories, 
                   'directoryListingCategories' => $directoryListingCategories->paginate(5) 
               ]);
    }
